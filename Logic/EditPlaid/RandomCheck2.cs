...
{
    public class RandomCheck2 : PlaidInterface
    {
        public SKBitmap EditImage(SKBitmap bmp, PlaidModel model)
        {
            ...
            int colorFlg = 0;
            int countFlg = BussinessLogic.GetRandomInt(model.VerticalSize1 / 3);
            ...
            for (int i = 0; i < model.VerticalSize1; i++)
            {
                ...
                if (countFlg == i)
                {
                    countFlg += BussinessLogic.GetRandomInt(model.VerticalSize1 / 3);
            ...
            for (int j = 0; j < model.HorizontalSize1; j++)
            {
                ...
                if (countFlg == j)
                {
                    countFlg += BussinessLogic.GetRandomInt(model.HorizontalSize1 / 3);
                    colorFlg = BussinessLogic.GetRandomInt(3);
                }
            }
