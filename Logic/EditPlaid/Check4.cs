...
{
    public class Check4 : PlaidInterface
    {
        public SKBitmap EditImage(SKBitmap bmp, PlaidModel model)
        {
            ...
            for (int x = 0; x < width; x++)
            {
                bool wflg1 = false;
                bool wflg2 = false;
                if (x % model.VerticalSize1 >= model.VerticalSize1 / model.BaseBalance / 2)
                {
                    if (wCount >= model.VerticalSize1 / model.BaseBalance / 2)
                    {
                        wCount = 0;
                        if (wCountFlg)
                        {
                ...
                for (int y = 0; y < height; y++)
                {
                    bool hflg1 = false;
                    bool hflg2 = false;
                    if (y % model.HorizontalSize1 >= model.HorizontalSize1 / model.BaseBalance / 2)
                    {
                        if (hCount >= model.HorizontalSize1 / model.BaseBalance / 2)
                        {
                            hCount = 0;
                            if (hCountFlg)
                            {
