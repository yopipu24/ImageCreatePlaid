...
{
    public class RandomCheck3 : PlaidInterface
    {
        public SKBitmap EditImage(SKBitmap bmp, PlaidModel model)
        {
            ...
            for (int i = 0; i < model.VerticalSize1; i++)
            {
                for (int j = 0; j < model.HorizontalSize1; j++)
                {
                    ...
                    {
                        originalBmp.SetPixel(i, j, BussinessLogic.GetMixColor(color1v, color1v));
                    }
            ...
            for (int j = 0; j < model.HorizontalSize1; j++)
            {
                for (int i = 0; i < model.VerticalSize1; i++)
                {
                    ...
                    {
                        originalBmp.SetPixel(i, j, BussinessLogic.GetMixColor(color2h, color2h));
                    }
