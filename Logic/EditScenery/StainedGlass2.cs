...
namespace ImageCreatePlaid
{
    public class StainedGlass2 : SceneryInterface
    {
        ...
        private static StainedCellModel FindNearestCell(int x, int y, List<StainedCellModel>[,] grid, int gridSize)
        {
            ...
            for (int yy = Math.Max(0, gy - 1); yy <= Math.Min(gridHeight - 1, gy + 1); yy++)
            {
                for (int xx = Math.Max(0, gx - 1); xx <= Math.Min(gridWidth - 1, gx + 1); xx++)
                {
                    foreach (StainedCellModel cell in grid[xx, yy])
                    {
                        int dx = x - cell.X;
                        int dy = y - cell.Y;
